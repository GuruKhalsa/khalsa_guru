		...
		def place_ship(ship)
			...
			ship.length.times do |distance|
				if cardinal_direction.match(/^[NS]$/)
					...
					fill_cell(origin_column, (origin_row.method(direction).(distance)), '#000')
					...
					fill_cell((origin_column.method(direction).(distance)), origin_row, '#000')
					...
					if @board[row_index][col_index] == '@'
						...
						@ships.each do |ship|
							...
							if ship.hits == ship.length
								...
								log.prepend('h4', "You Lose!") if @ships.all?{|ship| ship.sunk}
							end
						end
